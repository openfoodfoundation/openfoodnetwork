...
RSpec.describe "shopping with variant overrides defined", feature: :inventory do
  ...
  let(:product1) { create(:simple_product, enterprise_id: producer.id) }
  let(:product2) { create(:simple_product, enterprise_id: producer.id) }
  let(:product3) { create(:simple_product, enterprise_id: producer.id, on_demand: true) }
  let(:product4) { create(:simple_product, enterprise_id: producer.id) }
  let(:product1_variant1) {
    create(:variant, product: product1, price: 11.11, unit_value: 1, enterprise: producer)
  }
  let(:product1_variant2) {
    create(:variant, product: product1, price: 22.22, unit_value: 2, enterprise: producer)
  }
  let(:product2_variant1) {
    create(:variant, product: product2, price: 33.33, unit_value: 3, enterprise: producer)
  }
  let(:product1_variant3) {
    create(:variant, product: product1, price: 44.44, unit_value: 4, enterprise: producer)
  }
