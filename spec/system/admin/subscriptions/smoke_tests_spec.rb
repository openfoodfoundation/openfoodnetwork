...
RSpec.describe 'Subscriptions' do
  ...
  context "as an enterprise user" do
    ...
    describe "with an inactive order cycle" do
      ...
      let!(:product1) { create(:product, enterprise_id: shop.id) }
      let!(:product2) { create(:product, enterprise_id: shop.id) }
      let!(:variant1) {
    ...
    describe "allowed variants" do
      ...
      let!(:shop_product) { create(:product, enterprise_id: shop.id) }
      let!(:shop_product2) { create(:product, enterprise_id: shop.id) }
      let!(:shop_variant) {
        create(:variant, product: shop_product, unit_value: "2000", enterprise: shop)
      }
      let!(:shop_variant2) {
        create(:variant, product: shop_product2, unit_value: "1000", enterprise: shop)
      }
