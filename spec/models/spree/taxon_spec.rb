  ...
  RSpec.describe Taxon do
    ...
    describe ".supplied_taxons" do
      ...
      context "when scoped to specific enterprises" do
        it "finds taxons" do
          expect(Taxon.supplied_taxons([e.id])).to eq(e.id => Set.new([t1.id]))
          expect(Taxon.supplied_taxons([e2.id])).to eq(e2.id => Set.new([t2.id]))
          expect(Taxon.supplied_taxons([e.id, e2.id])).to eq(
            e.id => Set.new([t1.id]),
            e2.id => Set.new([t2.id])
          )
        end
      ...
      context "when not scoped to specific enterprises" do
        it "finds taxons" do
          expect(Taxon.supplied_taxons).to eq(
            e.id => Set.new([t1.id]),
            e2.id => Set.new([t2.id])
          )
        end
    ...
    describe ".distributed_taxons" do
      ...
      context "when scoped to specific enterprises" do
        it "finds all distributed taxons" do
          expect(Taxon.distributed_taxons(:all, [e.id])).to eq(e.id => Set.new([t1.id, t2.id]))
          expect(Taxon.distributed_taxons(:all, [e2.id])).to eq(e2.id => Set.new([t1.id, t2.id]))
          expect(Taxon.distributed_taxons(:all, [e.id, e2.id])).to eq(
        ...
        it "finds currently distributed taxons" do
          expect(Taxon.distributed_taxons(:current, [e.id])).to eq(e.id => Set.new([t1.id]))
          expect(Taxon.distributed_taxons(:current, [e2.id])).to eq(e2.id => Set.new([t1.id]))
          expect(Taxon.distributed_taxons(:current, [e.id, e2.id])).to eq(
            e.id => Set.new([t1.id]),
            e2.id => Set.new([t1.id]),
          )
        end
      ...
      context "when not scoped to specific enterprises" do
        it "finds all distributed taxons" do
          expect(Taxon.distributed_taxons(:all)).to eq(
            e.id => Set.new([t1.id, t2.id]),
            e2.id => Set.new([t1.id, t2.id]),
          )
        end
        ...
        it "finds currently distributed taxons" do
          expect(Taxon.distributed_taxons(:current)).to eq(
            e.id => Set.new([t1.id]),
            e2.id => Set.new([t1.id]),
          )
        end
