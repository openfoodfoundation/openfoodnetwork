...
describe I18nHelper, type: :helper do
  ...
  context "as guest" do
    ...
    it "sets the chosen locale" do
      allow(helper).to receive(:raw_params) { { locale: "es" } }
      helper.set_locale
    ...
    it "remembers the chosen locale" do
      ...
      allow(helper).to receive(:raw_params) { {} }
      helper.set_locale
      expect(I18n.locale).to eq :es
    end

    it "ignores unavailable locales" do
      allow(helper).to receive(:raw_params) { { locale: "xx" } }
      helper.set_locale
      expect(I18n.locale).to eq :en
    end

    it "remembers the last chosen locale" do
      allow(helper).to receive(:raw_params) { { locale: "en" } }
      helper.set_locale
