...
' do
  ...
  describe "Xero invoices report" do
    ...
    describe "with adjustments" do
      ...
      let!(:adj_shipping) { create(:adjustment, adjustable: order1.shipment, label: "Shipping", source: shipping_method, amount: 100.55, included_tax: 10.06) }
      let!(:adj_fee1) { create(:adjustment, adjustable: order1, source: enterprise_fee1, label: "Enterprise fee untaxed", amount: 10, included_tax: 0) }
      let!(:adj_fee2) { create(:adjustment, adjustable: order1, source: enterprise_fee2, label: "Enterprise fee taxed", amount: 20, included_tax: 2) }
      let!(:adj_manual1) { create(:adjustment, adjustable: order1, source: nil, label: "Manual adjustment", amount: 30, included_tax: 0) }
      let!(:adj_manual2) { create(:adjustment, adjustable: order1, source: nil, label: "Manual adjustment", amount: 40, included_tax: 3) }
