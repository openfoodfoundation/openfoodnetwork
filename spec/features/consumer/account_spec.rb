    ...
    context "with completed orders" do
      ...
      context "when there is at least one changeable order" do
        ...
        it "shows such orders in a section labelled 'Open Orders'" do
          ...
          expect(page).to have_link d1o1.number, href: main_app.order_path(d1o1)
          expect(page).to have_link d1o2.number, href: main_app.order_path(d1o2)
          expect(page).to have_link(distributor1.name,
                                    href: "#{distributor1.permalink}/shop", count: 2)
          expect(page).to have_link I18n.t('spree.users.open_orders.cancel'), href: main_app.cancel_order_path(d1o1)
          expect(page).to have_link I18n.t('spree.users.open_orders.cancel'), href: main_app.cancel_order_path(d1o2)
        end
      end
    end
