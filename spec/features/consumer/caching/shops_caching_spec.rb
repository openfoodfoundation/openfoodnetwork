...
feature "Shops caching", js: true, caching: true do
  ...
  describe "API action caching on taxons and properties" do
    ...
    let(:taxons_key) { "views/#{test_domain}/api/legacy/order_cycles/#{order_cycle.id}/taxons.json?distributor=#{distributor.id}" }
    let(:properties_key) { "views/#{test_domain}/api/legacy/order_cycles/#{order_cycle.id}/properties.json?distributor=#{distributor.id}" }
