describe "BulkProducts service", ->
  ...
  describe "cloning products", ->
    it "clones products using a http post request to /api/products/(id)/clone", ->
      ...
      $httpBackend.expectPOST("/api/legacy/products/13/clone").respond 201,
        id: 17
      $httpBackend.expectGET("/api/legacy/products/17?template=bulk_show").respond 200, [
        id: 17
      ]
    ...
    it "adds the product", ->
      ...
      $httpBackend.expectPOST("/api/legacy/products/16/clone").respond 201, clonedProduct
      $httpBackend.expectGET("/api/legacy/products/17?template=bulk_show").respond 200, clonedProduct
      BulkProducts.cloneProduct BulkProducts.products[0]
      $httpBackend.flush()
