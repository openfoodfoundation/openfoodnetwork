describe "EnterpriseRegistrationService", ->
  ...
  describe "creating an enterprise", ->
    describe "success", ->
      beforeEach ->
        ...
        $httpBackend.expectPOST("/api/legacy/enterprises?token=keykeykeykey").respond 200, 6
    ...
    describe "failure", ->
      beforeEach ->
        spyOn(RegistrationServiceMock, "select")
        spyOn(window, "alert")
        $httpBackend.expectPOST("/api/legacy/enterprises?token=keykeykeykey").respond 400, 6
        EnterpriseRegistrationService.create()
        $httpBackend.flush()
  ...
  describe "updating an enterprise", ->
    ...
    describe "success", ->
      beforeEach ->
        $httpBackend.expectPUT("/api/legacy/enterprises/78?token=keykeykeykey").respond 200, 6
    ...
    describe "failure", ->
      beforeEach ->
        spyOn(window, "alert")
        $httpBackend.expectPUT("/api/legacy/enterprises/78?token=keykeykeykey").respond 400, 6
        EnterpriseRegistrationService.update('step')
        $httpBackend.flush()
