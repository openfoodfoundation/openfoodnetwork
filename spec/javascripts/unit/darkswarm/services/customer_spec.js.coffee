describe 'Customer', ->
  describe "update", ->
    ...
    it "nests the params inside 'customer'", ->
      $httpBackend
        .expectPUT('/api/legacy/customers/3.json', { customer: { id: 3 } })
    ...
    describe "when the request succeeds", ->
      it "shows a success flash", ->
        $httpBackend.expectPUT('/api/legacy/customers/3.json').respond 200, response
        customer.update()
        $httpBackend.flush()
    ...
    describe "when the request fails", ->
      it "shows a error flash", ->
        $httpBackend.expectPUT('/api/legacy/customers/3.json').respond 400, { error: 'Some error' }
        customer.update()
        $httpBackend.flush()
        expect(RailsFlashLoaderMock.loadFlash)
