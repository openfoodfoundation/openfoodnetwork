...
describe OrderTaxAdjustmentsFetcher do
  describe "#totals" do
    ...
    let(:tax_rate10)      { create(:tax_rate, included_in_price: true, calculator: Calculator::DefaultTax.new, amount: 0.1, zone: zone) }
    let(:tax_rate15)      { create(:tax_rate, included_in_price: true, calculator: Calculator::DefaultTax.new, amount: 0.15, zone: zone) }
    let(:tax_rate20)      { create(:tax_rate, included_in_price: true, calculator: Calculator::DefaultTax.new, amount: 0.2, zone: zone) }
    let(:tax_rate25)      { create(:tax_rate, included_in_price: true, calculator: Calculator::DefaultTax.new, amount: 0.25, zone: zone) }
    ...
    let(:enterprise_fee)  { create(:enterprise_fee, enterprise: coordinator, tax_category: tax_category20, calculator: Calculator::FlatRate.new(preferred_amount: 48.0)) }
    ...
    let(:shipping_method) { create(:shipping_method, calculator: Calculator::FlatRate.new(preferred_amount: 46.0)) }
