class MigrateNamespacedPolymorphicModels < ActiveRecord::Migration
  ...
  def up
    update_column_data(%w(spree_payments spree_adjustments spree_log_entries), 'source_type')
    ...
    update_column_data(%w(spree_activators spree_assets spree_calculators spree_configurations spree_gateways spree_payment_methods), 'type')
    update_column_data('spree_assets', 'viewable_type')
    update_column_data('spree_zone_members', 'zoneable_type')
  end

  def down
    replace_column_data(%w(spree_payments spree_adjustments spree_log_entries), 'source_type')
    ...
    replace_column_data(%w(spree_activators spree_assets spree_calculators spree_configurations spree_gateways spree_payment_methods), 'type')
