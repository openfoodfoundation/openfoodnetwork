...
class DatabaseRestoreService
  ...
  def download_latest_backup
    ...
    @backup_file = bucket.objects.sort_by { |obj| obj.last_modified }.last
  ...
  def create_new_database
    @new_db_name = "#{ENV.fetch('OFN_DB_NAME', 'openfoodnetwork')}_restored_#{Time.now.strftime('%Y%m%d%H%M%S')}"
    system("psql -c 'CREATE DATABASE #{@new_db_name}' -h #{ENV.fetch('OFN_DB_HOST', 'localhost')} -U #{ENV.fetch('OFN_DB_USERNAME', 'ofn')} -d postgres")
    Rails.logger.info "New database created: #{@new_db_name}"
  end

  def restore_database_from_dump
    restore_command = "PGPASSWORD=#{ENV.fetch('OFN_DB_PASSWORD', 'f00d')} pg_restore -d #{@new_db_name} -h #{ENV.fetch('OFN_DB_HOST', 'localhost')} -U #{ENV.fetch('OFN_DB_USERNAME', 'ofn')} #{@local_path}"
