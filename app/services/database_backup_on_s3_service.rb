...
class DatabaseBackupOnS3Service
  def call
    ...
    pg_dump_command = "PGPASSWORD=#{ENV.fetch('OFN_DB_PASSWORD', 'f00d')} pg_dump -Fc -Z9 -U #{ENV.fetch('OFN_DB_USERNAME', 'ofn')} -h #{ENV.fetch('OFN_DB_HOST', 'localhost')} open_food_network_dev > #{object_key}" # rubocop:disable Layout/LineLength: Line is too long.

    puts pg_dump_command
