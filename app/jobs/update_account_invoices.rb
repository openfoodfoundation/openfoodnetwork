class UpdateAccountInvoices
  ...
  def update(account_invoice)
    ...
    if account_invoice.order.complete?
      Bugsnag.notify(RuntimeError.new('InvoiceAlreadyFinalized'),         invoice_order: account_invoice.order.as_json)
    else
  ...
  def clean_up(invoice_order, current_adjustments)
    ...
    if obsolete_adjustments.any?
      Bugsnag.notify(RuntimeError.new('Obsolete Adjustments'),         current: current_adjustments.map(&:as_json),
    ...
    if current_adjustments.empty?
      if invoice_order.persisted?
        Bugsnag.notify(RuntimeError.new('Empty Persisted Invoice'), invoice_order: invoice_order.as_json)
      else
  ...
  def settings_are_valid?
    unless end_date <= Time.zone.now
      Bugsnag.notify(RuntimeError.new('InvalidJobSettings'),         job: 'UpdateAccountInvoices',
