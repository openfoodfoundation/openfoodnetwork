class Exchange < ActiveRecord::Base
  ...
  scope :involving, ->(enterprises) { where('exchanges.receiver_id IN (?) OR exchanges.sender_id IN (?)', enterprises, enterprises).select('DISTINCT exchanges.*') }
  scope :supplying_to, ->(distributor) { where('exchanges.incoming OR exchanges.receiver_id = ?', distributor) }
  scope :with_variant, ->(variant) { joins(:exchange_variants).where('exchange_variants.variant_id = ?', variant) }
  scope :with_any_variant, ->(variants) { joins(:exchange_variants).where('exchange_variants.variant_id IN (?)', variants).select('DISTINCT exchanges.*') }
  scope :with_product, ->(product) { joins(:exchange_variants).where('exchange_variants.variant_id IN (?)', product.variants_including_master) }
