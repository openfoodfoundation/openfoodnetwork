class OrderCycle < ActiveRecord::Base
  ...
  scope :active, -> { where('order_cycles.orders_open_at <= ? AND order_cycles.orders_close_at >= ?', Time.zone.now, Time.zone.now) }
  scope :active_or_complete, -> { where('order_cycles.orders_open_at <= ?', Time.zone.now) }
  scope :inactive, -> { where('order_cycles.orders_open_at > ? OR order_cycles.orders_close_at < ?', Time.zone.now, Time.zone.now) }
  scope :upcoming, -> { where('order_cycles.orders_open_at > ?', Time.zone.now) }
  scope :closed, -> { where('order_cycles.orders_close_at < ?', Time.zone.now).order('order_cycles.orders_close_at DESC') }
  ...
  def suppliers
    enterprise_ids = exchanges.incoming.pluck :sender_id
    Enterprise.where('enterprises.id IN (?)', enterprise_ids)
  end

  def distributors
    enterprise_ids = exchanges.outgoing.pluck :receiver_id
    Enterprise.where('enterprises.id IN (?)', enterprise_ids)
  end
